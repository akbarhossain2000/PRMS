...
$id = $base->create_dep_id("time_schedule", "id");

?>
	<div class="msg"></div>
    <form action="" method="post" name="form" id="form">
    <input type="hidden" name="formcheck" value="time_schedule" />
	<table class="table">
    	<tr>
        	<th colspan="3" style="background-color:#0F9">Time Setup</th>
        </tr>
        ...
        <tr>
            ...
            <td>:</td>
            <td>
            	<select name="duid" id="duid" required><option>-------</option></select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>:</td>
            <td>
            	<select name="time_slot" id="time_slot" required><option>-------</option></select>
            </td>
        </tr>
		...
		$(document).ready(function(e) {
			...
			$.ajax({
				...
				success: function(data){
					...
					text += "<option value=''>-------</option>";
					for(id in data){
						text +="<option value='"+data[id].slot_id+"'>"+data[id].slot_name+" ("+data[id].start_time+" to "+data[id].end_time+")"+"</option>";
						//alert(text);
					}
					$('#time_slot').html(text);
				}
